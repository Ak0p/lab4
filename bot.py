...
song_list = {
    'DonToliver-CompanyPt2[Official Audio].webm' : '1',
    'ChildishGambino-FeelsLikeSummer.mp3' : '2',
    'KanyeWest-Hurricane.webm' : '3',
    'KanyeWest-Moon.webm' : '4',
    'NoRegrets(feat.DonToliver).opus' : '5'
}
...
async def play(ctx,song):
    if not ctx.message.author.voice:
        ...
        await ctx.send("How can I be of service?")
        ...
        try :
            ...
            if song in song_list:
                ...
                await ctx.send("I don't have that song...")
        except :
            await ctx.send("Nothing is being played")

@bot.command()
async def list(ctx):
    await ctx.send("Songs available:")
...
async def scram(ctx):
    ...
    if voice_client.is_connected():
        ...
        await ctx.send("I'll see you another time~!")
    else:
...
async def pause(ctx):
    ...
    if voice.is_playing():
        ...
        await ctx.send("I am not playing anything to begin with")
...
async def resume(ctx):
    ...
    if voice.is_paused():
        voice.resume()
        await ctx.send("The song is being resumed")
    else:
        await ctx.send("I can't resume while I'm playing something")
...
if __name__ == '__main__':
    ...
    if args.token:
        ...
        if 'BOT_TOKEN' not in os.environ:
            ...
            #if the args.token variable is empty then we use the environment variable BOT_TOKEN
#
